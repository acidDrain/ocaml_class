...
let make max_number_of_contacts =
  {number_of_contacts= 0; contacts= Array.make max_number_of_contacts nobody}
...
let search db contact =
  let rec aux idx =
    if idx >= db.number_of_contacts then (false, db, nobody)
...
let insert db contact =
  if db.number_of_contacts >= Array.length db.contacts then (false, db, nobody)
  else
...
let delete db contact =
  ...
  if not status then (false, db, contact)
    ...
    let cells i =
      if db.contacts.(i).name = contact.name then nobody else db.contacts.(i)
    in
    let db' =
      { number_of_contacts= db.number_of_contacts - 1
      ; contacts= Array.init (Array.length db.contacts) cells }
