...
let check_intersect (s1 : point) (s2 : point) : bool =
  ((s1.x -. s2.x) ** 2.0) +. ((s1.y -. s2.y) ** 2.0) +. ((s1.z -. s2.z) ** 2.0)
  <= get_sphere s2

let will_collide_soon (p1 : physical_object) (p2 : physical_object) : bool =
  let nextsphere1 = next p1 and nextsphere2 = next p2 in
  check_intersect nextsphere1.position nextsphere2.position
  || check_intersect nextsphere2.position nextsphere1.position
